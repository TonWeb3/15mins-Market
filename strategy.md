...
(fast feed)     (closed-form    (edge = fair − ask;       (gate +       (% or flat $  (FOK,
                GBM)            book stale?)               vetoes)       of balance)   paper/live)
```
...
3. **Veto filters** clear: not chasing an exhausted Heiken-Ashi streak (≥6 bars),
   not buying into RSI > 70 / selling into RSI < 30.

There is **no flat price cap** — EV governs reward/risk instead.
